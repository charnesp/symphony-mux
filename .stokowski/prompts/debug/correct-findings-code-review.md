# Correct findings from code review (debug workflow)

You are fixing issues identified in the last **`code-review`** run on a **bugfix** branch. Address the findings in the report / thread context; keep changes minimal and well-tested.
...
2. Address each point on the existing local branch (`fix/<identifier>-...`).
3. Run **`uv run pre-commit run --all-files`** and fix until all hooks pass (same bar as **`fix`** — not pytest/ruff alone).
