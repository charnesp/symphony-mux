# Route after automated code review (agent-gate) — debug workflow
...
**Transitions** — pick **exactly one** key for `"transition"` (same strings as below). Policy matches **`code-review`** (same severity rules as `prompts/feature/review.md`): **P0–P3** are merge blockers unless **explicitly accepted with documented rationale in that turn’s `<stokowski:report>`**; **P4** is never a merge blocker.
...
1. Ensure all intended work is **committed** on the **bugfix branch** `fix/<identifier>-...` (clear messages).
2. **Push** the branch: `git push -u origin <branch>` (set upstream if first push).
3. **Open** a PR/MR if none exists yet, and link the Linear issue in the description (e.g. `Fixes TEAM-123` or your project’s convention):
   ...
   # GitLab
   glab mr create --title "fix: ..." --description "..."
   ```
