...
def build_lifecycle_section(
    ...
    # CRITICAL: Report requirement FIRST with visual emphasis
    lines.append("## ⚠️ REQUIRED: Structured Work Report")
    lines.append("")
    lines.append(
        "**When you complete your work, you MUST include a structured work report "
        "wrapped in `<stokowski:report>` XML tags. Failure to include this report "
        "will result in your response being rejected and retried.**"
    )
    lines.append("")

    # Inject previous error for retry feedback
    if previous_error:
        lines.append("### 🔴 Previous Attempt Failed")
        lines.append("")
        lines.append("**Your previous response was rejected for the following reason:**")
        lines.append("")
        lines.append(f"> {previous_error}")
        lines.append("")
        lines.append("**You MUST correct this issue in your new response.**")
        lines.append("")

    lines.append("**Required format (place at the END of your response):**")
    lines.append("")
    lines.append("```xml")
    ...
    # Gate-specific report additions
    if has_gate_transition:
        lines.append("### Gate Review Required")
        lines.append("")
        lines.append(
            "**This state transitions to a GATE requiring human approval.** "
            "Include an 'Approval Required' section in your report:"
        )
        lines.append("")
        lines.append("```xml")
        lines.append("<stokowski:report>")
